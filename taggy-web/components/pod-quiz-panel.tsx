...
export function PodQuizPanel({
  ...
  async function beginQuiz() {
    ...
    if (next.status === "GENERATING") {
      setQuiz(next);
      setFeedback("AI is generating your quiz — this can take several minutes…");
      const ready = await pollQuizReady(next.id);
      if (!ready) {
        setBusy(false);
        return;
      }
    ...
    if (next.status === "FAILED") {
      setQuiz(next);
      setFeedback(null);
      showError("Quiz generation failed. Please try again.");
      return;
    }
    ...
    setSelected([]);
    setFeedback(null);
    const q1 = next.questions.find((q) => q.order_index === 1);
    setSecondsLeft(q1?.seconds_left ?? 60);
  }

  async function pollQuizReady(quizId: string): Promise<PodQuiz | null> {
    const maxAttempts = 180; // ~15 minutes at 5s
    for (let i = 0; i < maxAttempts; i++) {
      await new Promise((r) => setTimeout(r, 5000));
      const poll = await getPodQuiz(podSlug, quizId);
      if (!poll.ok || !poll.data) {
        showError(apiErrorMessage(poll));
        setQuiz(null);
        return null;
      }
      setQuiz(poll.data);
      if (poll.data.status === "IN_PROGRESS" || poll.data.status === "FAILED") {
        return poll.data;
      }
    }
    showError("Quiz generation is taking too long. Check back later or try again.");
    return null;
  }
