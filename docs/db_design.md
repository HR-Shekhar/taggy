...
A message must belong to exactly one location:
...
community_channel_id → CASCADE

pod_id → CASCADE
```

If a channel/pod is removed, associated messages disappear.
...
```text
Only members of a Community
can send messages to its channels.

Only members of a Pod
can send messages to that pod.
```
...
Notifications are user-specific and can be removed when the user is removed.
...
Only read/unread status changes.
```
